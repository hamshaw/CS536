...
int main(int argc, char const* argv[]){
	...
	hopefully_secret[6] = '\0';
        ...
        read(sock, &(forwardtab[sessionindex].destaddr[0]), sizeof(unsigned long));
	    read(sock, &(forwardtab[sessionindex].destpt[0]), sizeof(unsigned short));
        ...
        //if (pid == 0){//child code
	        ...
	        forwardtab[sessionindex].clientaddr = clientaddr;
            forwardtab[sessionindex].sendingaddrs[0] = fdaddr;

            int shouldfork = 1;
            for (int i = 0; i<NUMSESSIONS; i++){
                if (i == sessionindex) continue;
                if (forwardtab[i].sourceaddr == forwardtab[sessionindex].sourceaddr){
                    printf("Two connections from the same source.\n");
                    forwardtab[i].count = 1;
                    forwardtab[i].destaddr[1] = forwardtab[sessionindex].destaddr[0];
                    forwardtab[i].destpt[1] = forwardtab[sessionindex].destpt[0];
                    forwardtab[i].sendingaddrs[1] = forwardtab[sessionindex].sendingaddrs[0];
