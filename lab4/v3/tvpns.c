...
int main(int argc, char const* argv[]){
    ...
    address.sin6_port = htons(pn);
    address.sin6_scope_id = if_nametoindex("eth0");
	...
	hopefully_secret[6] = '\0';
        ...
        read(sock, &(forwardtab[sessionindex].destaddr), sizeof(struct in6_addr));
	read(sock, &(forwardtab[sessionindex].destpt), sizeof(unsigned short));
	read(sock, &(forwardtab[sessionindex].sourceaddr), sizeof(struct in6_addr));
        int pid = fork();
        if (pid == 0){//child code
            //do stuffi
            int new_sd;
            if ((new_sd = socket(AF_INET6, SOCK_DGRAM, 0)) < 0){
                printf("CREATING SOCKET FAILED\n");
                exit(-1);
            }
            struct sockaddr_in6 childaddr, fdaddr;

	    memset(&fdaddr, 0, sizeof(fdaddr));
            memset(&childaddr, 0, sizeof(childaddr));
	    
	    fdaddr.sin6_family = AF_INET6;
	    fdaddr.sin6_addr = forwardtab[sessionindex].destaddr;//CONTAINS FINAL DEST INFO 
	    fdaddr.sin6_port = forwardtab[sessionindex].destpt;
            
            childaddr.sin6_addr = in6addr_any;//CONTAINS servers port with which it will reach ping client
	    childaddr.sin6_family = AF_INET6;
	    fdaddr.sin6_scope_id = if_nametoindex("etho0");
	    childaddr.sin6_scope_id = if_nametoindex("eho0");
            
	    unsigned short port2 = 55500;
            for (int j = 0; j< 100; j++){//CHANGE TO WHILE(1)
                childaddr.sin6_port = htons(port2);
	    ...
	    //exit(1);//takeout
            int new_sd_out;
            if ((new_sd_out = socket(AF_INET6, SOCK_DGRAM, 0)) < 0){
                printf("CREATING SOCKET FAILED\n");
                exit(-1);
            }
            struct sockaddr_in6 childaddr_out;//CONTAINS SERVERS INFO, TO CONTACT FD
            memset(&childaddr_out, 0, sizeof(childaddr_out));
            childaddr_out.sin6_addr = in6addr_any;//forwardtab[sessionindex].destaddr;//sin_addr.s_addr?
            childaddr_out.sin6_family = AF_INET6;
            
	    int port = 57500;
	
            for (int p = 0; p<200; p++){
                childaddr_out.sin6_port = htons(port);
		...
		printf("select success\n");
		    ...
		    char hopefully_secret2[6];
                    ...
                    if (strncmp(secret, hopefully_secret2, 6) != 0){
                        ...
                        forwardtab[sessionindex].sourcept = 0;//make shared mem
                        close(new_sd);
                        close(new_sd_out);
                        close(sd);
			close(sock);
