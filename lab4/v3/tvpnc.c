...
int main(int argc, char const* argv[]){
    ...
    struct in6_addr ip_UDPc;
    inet_pton(AF_INET6, argv[4], &ip_UDPc);
    struct in6_addr ip_fd;//const char * ip_fd ip_fd = argv[5];
    inet_pton(AF_INET6, argv[5], &ip_fd);
    ...
    address.sin6_port = pn_ts;
    inet_pton(AF_INET6, ip_ts, &(address.sin6_addr));
    ...
    char int_secret[sizeof(unsigned int) + 6 + 2*sizeof(struct in6_addr)+ sizeof(unsigned short)] = {0};
    unsigned int code = 315;
    memcpy(int_secret, &code, sizeof(unsigned int));
    memcpy(int_secret+(sizeof(unsigned int)), secret, 6);
    memcpy(int_secret+6+(sizeof(unsigned int)), &ip_fd, sizeof(struct in6_addr));
    memcpy(int_secret+6+sizeof(struct in6_addr)+(sizeof(unsigned int)), &pn_fd, sizeof(unsigned short));
    memcpy(int_secret+6 +sizeof(struct in6_addr) + sizeof(unsigned short)+(sizeof(unsigned int)), &ip_UDPc, sizeof(struct in6_addr));
    //write()
    write(sd, int_secret, sizeof(unsigned int) + 6 + sizeof(struct in6_addr)*2 + sizeof(unsigned short));
